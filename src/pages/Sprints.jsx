...
const SprintBoard = () => {
  ...
  const { sprints: data, setSprints } = useContext(OverallContext);
  ...
  const boardClasses = "-mt-4 sm:-mt-6 lg:-mt-8 -ml-4 sm:-ml-6 lg:-ml-8 -mr-4 sm:-mr-6 lg:-mr-8 pt-0 overflow-x-hidden";
  const headerClasses = "text-4xl font-bold mb-4 mx-60 bg-white p-4 w-4/5";
  const containerClasses = "flex flex-wrap gap-4 mx-80 w-4/5";
  const columnClasses = "w-full md:w-[315px] h-[190] rounded-lg flex flex-col gap-4";
    ...
    <div className={boardClasses}> 
      <div className='mx-10 w-[120vw]'>
        <h1 className={headerClasses}>
          Sprint Overview <span className="text-gray-400">0001</span>
        </h1>
      </div>
          ...
          <div key={status} className={columnClasses}>
            ...
            {cards.map((card, idx) => (
              ...
              >
                ...
                {card.footer && <p className="text-xs text-gray-600 pt-1">{card.footer}</p>}
